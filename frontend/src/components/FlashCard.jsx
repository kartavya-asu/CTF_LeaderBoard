...
const Flashcard = ({ challenge, onHintToggle, onAnswerSubmit, onSetAnswer, answer, attemptsLeft }) => {
  ...
  <div className="actions flex justify-around my-4">
    <button onClick={handleHintToggle} className="hint-toggle btn bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
      {showHint ? 'Hide Hint' : 'Show Hint'} (-3 points)
    </button>
    <a href={`/data/${challenge.filename}`} download className="download btn bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
      Download File
    </a>
  </div>
  <div className="answer-section">
    ...
    />
    <button onClick={onAnswerSubmit} className="submit-answer btn mt-4 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded block w-full">
      Submit
    </button>
